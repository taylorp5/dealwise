...
Respond with JSON: {"intent": "...", "confidence": 0.0-1.0, "redirectTo"?: "..."}`},{role:"user",content:e}],temperature:.3,max_tokens:50}),i=t.choices[0]?.message?.content||"{}",o=JSON.parse(i);return{intent:o.intent||"decision",confidence:o.confidence||.7,redirectTo:o.redirectTo}}catch(e){return{intent:"decision",confidence:.5}}}async function l(e){try{let{messages:t,context:i,mode:o}=await e.json();if(!t||!Array.isArray(t)||0===t.length)return new Response(JSON.stringify({error:"Missing or invalid messages array"}),{status:400,headers:{"Content-Type":"application/json"}});let r=t[t.length-1];if("user"!==r.role)return new Response(JSON.stringify({error:"Last message must be from user"}),{status:400,headers:{"Content-Type":"application/json"}});let s=await c(r.content,t),a=function(e,t,i){let o=`LISTING CONTEXT:
...
`)),e.close()}}});return new Response(p,{headers:{"Content-Type":"text/event-stream","Cache-Control":"no-cache",Connection:"keep-alive"}})}catch(e){return console.error("Advisor chat error:",e),new Response(JSON.stringify({error:e.message||"Internal server error"}),{status:500,headers:{"Content-Type":"application/json"}})}}let d=new r.AppRouteRouteModule({definition:{kind:s.x.APP_ROUTE,page:"/api/advisor/chat/route",pathname:"/api/advisor/chat",filename:"route",bundlePath:"app/api/advisor/chat/route"},resolvedPagePath:"C:\\dev\\Dealership Copilot\\app\\api\\advisor\\chat\\route.ts",nextConfigOutput:"",userland:o}),{requestAsyncStorage:u,staticGenerationAsyncStorage:p,serverHooks:h}=d,f="/api/advisor/chat/route";function m(){return(0,a.patchFetch)({serverHooks:h,staticGenerationAsyncStorage:p})}},49303:(e,t,i)=>{e.exports=i(30517)}};var t=require("../../../../webpack-runtime.js");t.C(e);var i=e=>t(t.s=e),o=t.X(0,[8948,4214],()=>i(72864));module.exports=o})();